...
## Action Plan
Use ML model to determine which features/ factors best determine higher math score achievement.
...
## SQL Work
  - Please refer to files table_joins.sql and QuickDBD-export.sql. Utilized Quick Db website to assist with ERD compilation. Then used the export code from QuickDB to build tables in SQL.
...
## EDA
  ...
  - Based on our definition of a "passing" math score (being 70 or higher) I then extracted a data framef showing just the passing students. In Addition, I created a function to print the outliers of the Math_score" column, which when applied to te data set above found 6 statistical outliers that were below the lower_limit/ q1
...
## Mock Logistic_Regression
 - Please refer to "Mock_Logistic_Regression.ipynb". After analyzing the data, my next task was to apply the data set to an initial Logistic regression model to see if/ how efficient we would be able to determine the math scores of the students based on the features/ columns in the Student_exams data set/ table. the initial step taken for this was to convert the "Math_Score" column in to 1's and 0's based on the students score in relation to being above/ below a score of 70. Where a score of 70 or greater was a 1 and anything below 70 was a 0.Next i created my features and placed in the X variable, and the target feature (Math_score) in to the y variable. I Double checked the "X" dtypes and the value_counts of "y". Once determined that the variables were correctly allocated, I instantiated the train_test_split and applied random over sampling to the data set. Next I trained the model on the data set and calculated the results (which can be seen below).
...
## Mock Balanced_Random_forrest/ Feature_Importances
 - In addition to the Logistic regression model I wanted to test a balanced random forest model on the data to compare accuracy scores and to also retrieve the feature importance. I felt the feature importance’s could be an important detail in answering our initial question of determining which features best determined the math test scores. The coed in the "Bala_Rando_Forest.ipynb file essentially was initially boiler plate when compared to the logistic regression code. Converting the Math_Score column, creating the features and targets, and the train_test_split. Then instead of random_over_sampling to sample the data, I applied a balanced Random Forest Classifier to obtain the balanced accuracy score and Geometric mean. Please see the results below:
 
 ![image](https://user-images.githubusercontent.com/117245167/233061911-5e9470fa-c455-4e19-965d-67fc94470d3d.png)

 - My last step was to pull out and inspect the feature importance’s of the data set to help determine which features impacted the predictability the most, which can be seen below:
...
## Mock Easy_Ensamble_AdaBoost
 - In Another attempt to compare models I wanted to test our data against an additional supervised Machine learning model. I did not optimize the data in any way and wanted to see what results we could get trying to predict the match scores using the Easy Ensemble AdaBoost method. The data was pre-processed very similarly to the previous two models and imported via the connection string linked to my SQL/ PGadmin account. The results below were similar to the balanced Random forest classifier, but not as effective as the logistic regression model.
...
## Mock Gradient Boost
 - The gradient boosted model produced similar results but came up slightly short when compared to the Logistic regression and SVM models. You can see the optimization attempts in the optimization_trial_error folder.
 
 ![image](https://user-images.githubusercontent.com/117245167/235139474-67215baf-fe2a-4d99-9a51-4b22026c4b4e.png)

## Mock Decision Tree
 - The Decision tree model yielded an accuracy score of 0.78 which when compared to the stronger models does not hold up.
...
### Optimizing the model based on EDA
 - As a group we all worked on the EDA process and after our mock machine learning trials and applying them to our data, we discovered a couple of optimization opportunity’s. First, we established the outliers in our data set. There were a total of 9 scores in our data set between reading, writing, and math that feel below the lower Q1 limit (which can be seen above). Once these were identified we removed these from the data sets we used when optimizing our models. In addition, we ran feature correlation analysis on the 3 continuous features (math, reading, writing scores) and it was determined the reading and writing scores were statistically very similar to each other. With this being determined we decided to remove the weaker of the two in determining the math score, which was the Writing_Score.

## Hyperparameter Tuning
 - After analyzing the results of our mock machine learning models, we narrowed down the options for the model that we planned on using for our project. Based on the initial Hyperparameter tuning, [Initial_Hyperparameter_tuning](https://github.com/Jess-Vannatter/Education_Group_Project-/blob/Jess_Dev/Hyperparameter_tuning/Initial_Hyperparameter_tuning.ipynb) the two top preforming models were the Logistic regression model and the GB_Classifier model. We have an initial Hyperparameter tuning file which shows a side by side comparison of the mock models[Initial_Hyperparameter_tuning](https://github.com/Jess-Vannatter/Education_Group_Project-/blob/Jess_Dev/Hyperparameter_tuning/Initial_Hyperparameter_tuning.ipynb) , then a Hyperparameter tuning file which only compares the strongest models [Hyperparameter_tuning_best](https://github.com/Jess-Vannatter/Education_Group_Project-/blob/Jess_Dev/Hyperparameter_tuning/Hyperparameter_tuning_best_models.ipynb) , Logistic and GB_Classifier. Both are located in my "Hyperparmeter_Tuning file".

### Grid_Hyperpara_Optimized_Logistic_Regression_Model
 - Initially I had tried to optimize this model by manually changing the hyperparameters which took a long time and was not effective in yielding a higher accuracy score. but after some research i was able to dig into a couple of approaches that would cut down on time and allow us to run through essentially of the possible hyperparameter tuning by utilizing "GridSearchCV". This tool allows us to provide all the possible parameters in our code then by using GridSearchCV we apply our model to or data set and test all hyperparameter options to determine which sequence will yield the best accuracy results [Grid_Hyperpara_Optimized_Logistic_Regression_Model](https://github.com/Jess-Vannatter/Education_Group_Project-/blob/Jess_Dev/Hyperparameter_tuning/Grid_Hyperpara_Optimized_Logistic_Regression_Model.ipynb)  After utilizing the GridSearchCVand testing the different Hyperparameters:
 ```
 param_grid = [    
...
```
We were able to come to the conclusion that our best possible sequence was: ```LogisticRegression(C=1.623776739188721, solver='liblinear')``` .Which yielded a 0.891 accuracy score.
 
### Hypertune_G_Boost
 - Similarly to the Logistic model, We utilized griSearchCV to tune the Hyperparameters for the GB_classifier model as well. As compared to the logistic model though, The GB_Classifier had significantly more parameter options to choose from, or at least the range of which the parameters could fall in:
 ```
 parameters = {
...
```
Using the parameters above with the GridSearchCV approach did take a decent amount of computing power and took over an hour to run. This resulted in our best sequence of GB_Classifier hyperparameters being ```Best parameters are: {'learning_rate': 0.1, 'max_depth': 1, 'max_features': 4, 'n_estimators': 250}``` and an accuracy score of 0.8842. You can see my hypertune process in the [Hypertune__Boost file](https://github.com/Jess-Vannatter/Education_Group_Project-/blob/Jess_Dev/Hyperparameter_tuning/Hypertune_G_Boost.ipynb) and the optimized model in the [Optimized_G_Boost](https://github.com/Jess-Vannatter/Education_Group_Project-/blob/Jess_Dev/Optimized_G_Boost.ipynb).

![image](https://user-images.githubusercontent.com/117245167/235942016-2cfffca3-2ef5-4ce6-9243-e5b007ea12c3.png)

## Optimized Logistic Regression Model and Feature Importance’s
 - We Chose the logistic regression model because it yielded the highest accuracy score out of the 6 initial mock models tested and in our Hyperparameter tuning trials. In order to optimize the model we really didn’t have many options since the data was already relatively cleaned and balanced. What we were able to do is remove all outliers identified in the EDA process and try to see if removing the resampling process had any impact to the overall accuracy score. Initially I just removed the outliers for the Math Scores, but that did not yield a positive outcome (accuracy score slightly lowered). By removing the outliers from all scoring features, including Math, Reading, and Writing we were able to achieve an accuracy ~ 90%. Although it was determined that re-sampling was not needed considering our data set we relatively balanced as is. After the Hyperparameter tuning we were able to achieve ~89% accuracy score which was the highest we achieved between all models.

![image](https://user-images.githubusercontent.com/117245167/235442340-248706d4-f32f-40d1-ba23-f8ddead1d052.png)

### Feature Importance’s
 - As a part of this analysis it was our goal to determine which features/ factors would best determine Math scores at or above the achievement threshold of 70. So, in order to pin point the features that had the most impact on the math score prediction also obtained the "Feature Importance’s" of the Logistic Regression by generating the normalized coefficients of each feature in the "X" array. Then printed the coefficients, along with their "Feature Names" in ascending order. See below, as well as in our Resources folder under the Feature_Importances.csv file.
...
![Screenshot 2023-05-01 192959](https://user-images.githubusercontent.com/117245167/235549199-82ff53e2-9e90-4701-ad70-db7bfb7f058d.png)
 ...
 - Once the feature importance’s were determined we attempted to further optimize our model by cultivating the feature importance’s via a feature selection process. We tried removing feature importance’s with "importance" lower than 0.4 and the again at .02. Lastly, we tried running the same logistic regression model after removing the 2 least important features as presented above but were not able to achieve an accuracy score greater than 86%. So, it was determined that the features in the list above should remain in the model. You can see these attempts in my "Optimization_Trial_Error" folder.

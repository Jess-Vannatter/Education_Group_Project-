...
We chose the Logistic Regression model because it yeilded the highest accuracy score out of the 6 initial mock models tested and in our Hyperparameter tuning trials. In order to optimize the model, we had limited options since the data was already relatively cleaned and balanced. We were able to remove all outliers identified in the EDA process and determine if removing the resampling process had any impact on the overall accuracy score. Iniitlaly just the outliers for the Math Scores were removed, but that did not yield a positive outcome (accuracy score slightly lowered). By removing the outliers from all scoring features, including Math, Reading, and Writing, we were able to achieve an accuracy ~ 90%. Although it was determined that re-sampling was not needed considering our data set we relativly balanced as is. After the Hyperparameter tuning, we were able to achieve ~89% accuracy score which was the highest wwe achieved between all models.

![image](https://user-images.githubusercontent.com/117245167/235442340-248706d4-f32f-40d1-ba23-f8ddead1d052.png)

One of our goals with this project was to determine which features/ factors would best predict math scores at or above the achievement threshold of 70. So in order to identify the features most impacting the math score prediction, we also obtained the "Feature Importances" of the Logistic Regression by generating the normalized coefficientsof each feature in the "X" array. Then the coefficients were printed, along with their "Feature Names" in descending order as seen below, as well as in our Resources folder under the Feature_Importances.csv file.
